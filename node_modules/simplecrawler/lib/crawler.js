...
Crawler.prototype.start = function() {
    ...
    var length = crawler.queue.getLength();
    if (!length) {
...
*/
Crawler.prototype.queueURL = function(url, queueItem, meta) {
    var crawler = this,
...
Crawler.prototype.fetchQueueItem = function(queueItem) {
    ...
    clientRequest =
        client.request(requestOptions, function(response) {
            clearTimeout(queueItem.reqTimeout);
            crawler.handleResponse(queueItem, response, timeCommenced);
        });

    queueItem.reqTimeout = setTimeout(function() {
...
Crawler.prototype.handleResponse = function(queueItem, response, timeCommenced) {
    ...
    // We should just go ahead and get the data
        ...
        downloadDenied = crawler._downloadConditions.reduce(function(prev, callback) {
            return prev || !callback(queueItem);
        }, false);
    ...
    // If we should queue a redirect
        ...
        crawler.queueURL(parsedURL, queueItem, queueItem.meta);
        response.socket.end();
...
Crawler.prototype.addDownloadCondition = function(callback) {
    var crawler = this;
    if (callback instanceof Function) {
        crawler._downloadConditions.push(callback);
        return crawler._downloadConditions.length - 1;
    }
